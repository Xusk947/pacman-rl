...
- `o` — синяя “большая” пилюля (power pellet, съедаемо)
- ` ` (space) — пустой проход
...
- Если Pacman попадает на клетку с `o` — power pellet исчезает и активируется режим “frightened” для призраков на `T` шагов (конфиг, например 40).
- В режиме “frightened” коллизия Pacman с призраком НЕ завершает эпизод: призрак считается “съеденным”, Pacman получает бонус-награду, а призрак респавнится на своей стартовой позиции (или в заранее заданной “ghost home” зоне, если она появится позже).

#### Scenario: Success case
- **WHEN** выполняется `step` с валидными действиями
- **THEN** позиции обновляются, пеллеты/большие пилюли собираются, коллизии учитываются с учётом frightened-режима, и возвращаются корректные награды и флаг завершения.
